...
namespace GoogleDriveClient
{
    public partial class client : Form
    {
        ...
        public client(string email, string password, int version, int revision)
        {
            ...
            browser.Load("https://accounts.google.com/signin/v2/identifier?service=wise&passive=true&continue=http%3A%2F%2Fdrive.google.com%2F%3Futm_source%3Den&utm_medium=button&utm_campaign=web&utm_content=gotodrive&usp=gtd&ltmpl=drive&flowName=GlifWebSignIn&flowEntry=ServiceLogin");
        }
        ...
        private void InputPassword_Tick(object sender, EventArgs e)
        {
                ...
                {
                    browser.ExecuteScriptAsync("document.getElementsByName(\"password\")[0].value='" + accPassword + "';");
                    browser.ExecuteScriptAsync("document.getElementById('passwordNext').click();");
                }

                if (browser.Address == "https://drive.google.com/drive/u/0/my-drive" || browser.Address == "https://drive.google.com/drive/my-drive")
                {
                    inputPassword.Stop();
                    loadCompleted = true;
        ...
        private void SignoutExit_Click(object sender, EventArgs e)
        {
            ...
            foreach (Form f in Application.OpenForms)
            {
                if (f is signInInfo)
                {
                    browser.ExecuteScriptAsync("document.getElementsByClassName('gb_ia gb_yg gb_i')[0].click();");
        ...
        {
            browser.ExecuteScriptAsync("window.open(\"https://mail.google.com/mail/?tab=om1\",'_blank');");
        }
